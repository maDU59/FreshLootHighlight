package fr.madu59.flh.config.configscreen.entries.builders;
...
import fr.madu59.flh.config.configscreen.MyConfigListWidget.ButtonEntry;
...
public class ButtonBuilder extends AbstractEntryBuilder{
    ...
    public void build() {
        button = Button.builder(Component.translatable(FreshLootHighlight.MOD_ID + ".config.value." + option.getValue().toString().toLowerCase()), btn -> {option.setToNextValue();}).bounds(0, 0, 100, 20).build();
